    ...
    <div class="d-flex justify-content-center align-items-center">
      ...
      >
        ...
        <div class="form-group mt-3">
          ...
          <select
            v-model="Direccion.selectedProvince"
            class="form-control"
            id="province"
            required
            @change="getCounties"
        ...
        <div class="form-group mt-3">
          ...
          <select
            v-model="Direccion.selectedCanton"
            class="form-control"
            id="canton"
            :disabled="!Direccion.selectedProvince"
            required
            @change="getDistricts"
          >
            <option value="" disabled selected>Seleccione un cantón</option>
            <option v-for="canton in cantones" :key="canton.value" :value="canton.value">
              {{ canton.value }}
            </option>
          </select>
        </div>
        ...
        <div class="form-group mt-3">
          ...
          <select
            v-model="Direccion.selectedDistrit"
            class="form-control"
            id="distrit"
            :disabled="!Direccion.selectedCanton"
            required
            @change="getZipCode"
          >
            <option value="" disabled selected>Seleccione un distrito</option>
            <option v-for="distrit in distritos" :key="distrit.value" :value="distrit.value">
              {{ distrit.value }}
            </option>
          </select>
        </div>
        ...
        <div
          class="d-flex flex-column justify-content-center align-items-center mt-4 mb-4 text-center"
          id="termsandlogin"
        >
          ...
          <div>
            ...
            <router-link to="/auth/Login" class="text-decoration-none"> Inicie Sesión </router-link>
          </div>
        </div>
  ...
  methods: {
    ...
    getCounties() {
      ...
      URLBaseAPI.get('/api/CountryDivision/Counties', {
        params: { province: this.Direccion.selectedProvince },
      })
        .then((response) => {
    ...
    getZipCode() {
      URLBaseAPI.get('/api/CountryDivision/ZipCode', {
        params: {
          province: this.Direccion.selectedProvince,
          county: this.Direccion.selectedCanton,
          district: this.Direccion.selectedDistrit,
        },
      })
        .then((response) => {
    ...
    async validateForm() {
      ...
      if (!this.Direccion.selectedProvince)
        this.errors.Provincia = 'Debe seleccionar una provincia';
      ...
      if (this.password !== this.confPassword)
        this.errors.ConfPassword = 'Las contraseñas no coinciden';
