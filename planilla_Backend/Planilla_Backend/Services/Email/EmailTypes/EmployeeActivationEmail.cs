...
{
  public class EmployeeActivationEmail : IEmailType
  {
    private readonly ActivationAccountModel _activationModel;
    public EmployeeActivationEmail(ActivationAccountModel activationModel)
    {
      _activationModel = activationModel;
    }
      ...
      return $@"
        <div style='font-family: Arial, sans-serif; background-color: #f5f5f5; padding:20px;'>
          <div style='max-width:600px; margin:0 auto; background-color:#ffffff; border-radius:10px; box-shadow:0 0 10px rgba(0,0,0,0.1); padding:40px; text-align:center;'>
            <h2 style='color:#333333;'>Hola {_activationModel.userName}, bienvenido al sistema ONIGIRI INTELLIGENT SOLUTIONS</h2>
            <p style='color:#555555; font-size:16px; line-height:1.5;'>
              Por favor, haga clic en el siguiente botón para activar su cuenta:
            </p>
            <a href='{_activationModel.activationLink}' style='display:inline-block; padding:15px 25px; background-color:#28a745; color:#ffffff; text-decoration:none; font-size:16px; border-radius:5px; margin:20px 0;'>Activar Cuenta</a>
