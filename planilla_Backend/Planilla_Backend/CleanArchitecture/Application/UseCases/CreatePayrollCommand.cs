...
namespace Planilla_Backend.CleanArchitecture.Application.UseCases
{
  public class CreatePayrollCommand : ICreatePayrollCommand
  {
    ...
    public async Task<PayrollSummary> Execute(int companyId, int personId, DateTime dateFrom, DateTime dateTo)
    {
      ...
      await _session.BeginAsync(IsolationLevel.Snapshot);

      try
      {
        var payrollExists = await _repo.ExistsPayrollForPeriod(companyId, dateFrom, dateTo, _session.Connection, _session.Transaction);
        if (payrollExists) throw new InvalidOperationException("Ya existe una planilla para el periodo seleccionado");

        var company = await _repo.GetCompany(companyId, _session.Connection, _session.Transaction);
        if (company == null || company.Id <= 0) throw new KeyNotFoundException("La empresa no existe");

        var employees = (await _repo.GetEmployees(companyId, dateFrom, dateTo, _session.Connection, _session.Transaction)).ToList();
        var contracts = (await _repo.GetContracts(companyId, dateFrom, dateTo, _session.Connection, _session.Transaction)).ToList();
        var taxes = (await _repo.GetTaxes(dateFrom, dateTo, _session.Connection, _session.Transaction)).ToList();
        var ccss = (await _repo.GetCCSS(dateFrom, dateTo, _session.Connection, _session.Transaction)).ToList();
        var hoursByEmployee = await _repo.GetEmployeeTimesheets(companyId, dateFrom, dateTo, _session.Connection, _session.Transaction);
        ...
        var elementsByEmployee = await BuildElementsMapAsync(companyId, employees, dateFrom, dateTo, _session.Connection, _session.Transaction);
        ...
        var companyPayrollId = await _repo.SaveCompanyPayroll(companyPayroll, _session.Connection, _session.Transaction);
        ...
        while (i < employees.Count)
        {
          ...
          var employeePayrollId = await _repo.SaveEmployeePayroll(employeePayroll, _session.Connection, _session.Transaction);
        ...
        var calculatedLines = _template.RunCalculation(companyId, dateFrom, dateTo, ctx) ?? new List<PayrollDetailModel>();
        if (calculatedLines != null && calculatedLines.Count > 0)
        {
          ...
          foreach (var group in detailsByPayroll)
          {
            ...
            await _repo.SavePayrollDetails(employeePayrollId, employeeDetails, _session.Connection, _session.Transaction);
          }
        }
        ...
        foreach (var kvp in totalsByPayroll)
        {
          ...
          await _repo.UpdateEmployeePayrollTotals(employeePayrollId, empModel, _session.Connection, _session.Transaction);
          ...
          await _repo.SavePayment(employeePayrollId, payment, _session.Connection, _session.Transaction);
        ...
        await _repo.UpdateCompanyPayrollTotals(companyPayrollId, companyPayroll, _session.Connection, _session.Transaction);
      ...
      {
        _logger.LogError(ex, "Error creando planilla. companyId={CompanyId}", companyId);
        await _session.RollbackAsync();
        throw;
      }
    }
    
    private async Task<IDictionary<int, IList<ElementModel>>> BuildElementsMapAsync(int companyId, IList<EmployeeModel> employees, DateTime dateFrom, DateTime dateTo, SqlConnection conn, SqlTransaction? tx)
    {
      var dict = new Dictionary<int, IList<ElementModel>>();
      foreach (var e in employees)
      {
        var list = await _repo.GetElementsForEmployee(
          companyId, e.Id, dateFrom, dateTo,
          conn, tx);

        dict[e.Id] = list.ToList();
      }
